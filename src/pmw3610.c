...
static bool pmw3610_is_scroll_layer(const struct device *dev) {
  const struct pixart_config *config = dev->config;
...
bool pmw3610_inertial_scroll_is_enabled(const struct device *dev) {
  ...
  return pmw3610_is_scroll_layer(dev);
}
...
bool pmw3610_vertical_scroll_direction_is_inverted(const struct device *dev) {
  ...
  if (!pmw3610_is_scroll_layer(dev)) {
    return false;
  }
...
static void pmw3610_update_inertia_from_motion(
  ...
  if (abs(init_vx) >= thresh || abs(init_vy) >= thresh) {
    ...
    k_work_reschedule(&data->inertia_work, K_MSEC(config->inertial_scroll_interval_ms));
  }
}
...
static void pmw3610_inertia_work_callback(struct k_work *work) {
  ...
  if (sx != 0 || sy != 0) {
    if (sx != 0) {
      input_report(dev, config->evt_type, config->x_input_code, sx, (sy == 0), K_NO_WAIT);
    }
    if (sy != 0) {
      input_report(dev, config->evt_type, config->y_input_code, sy, true, K_NO_WAIT);
    }
  }
...
#define PMW3610_DEFINE(n)                                                      \
    ...
    static const struct pixart_config config##n = {                            \
        ...
        .inertial_scroll_decay_pct =                                            \
            DT_PROP_OR(DT_DRV_INST(n), inertial_scroll_decay_pct, 99),         \
        .inertial_scroll_decay_basis_points =                                  \
            DT_PROP_OR(DT_DRV_INST(n), inertial_scroll_decay_basis_points, 0), \
        .inertial_scroll_interval_ms =                                         \
            DT_PROP_OR(DT_DRV_INST(n), inertial_scroll_interval_ms, 10),       \
        .inertial_scroll_threshold =                                           \
            DT_PROP_OR(DT_DRV_INST(n), inertial_scroll_threshold, 4),          \
        .inertial_scroll_gain_pct =                                            \
            DT_PROP_OR(DT_DRV_INST(n), inertial_scroll_gain_pct, 130),         \
        .inertial_scroll_max_velocity =                                        \
            DT_PROP_OR(DT_DRV_INST(n), inertial_scroll_max_velocity, 32),      \
        .inertial_scroll_max_duration_ms =                                     \
            DT_PROP_OR(DT_DRV_INST(n), inertial_scroll_max_duration_ms, 1800), \
        .inertial_scroll_fade_duration_ms =                                    \
            DT_PROP_OR(DT_DRV_INST(n), inertial_scroll_fade_duration_ms, 250), \
...
#define PMW3610_DEVICE_COUNT ARRAY_SIZE(pmw3610_devs)
